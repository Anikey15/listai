...
const PASSWORD_HASH_FILE = path.join(__dirname, "..", "data", "admin_password.hash");

function loadAdminPasswordHash() {
  // На Render символы $ в Environment часто ломаются — основной источник хеша: файл.
  try {
    if (fs.existsSync(PASSWORD_HASH_FILE)) {
      const fromFile = fs.readFileSync(PASSWORD_HASH_FILE, "utf8").trim();
      if (fromFile.startsWith("$2")) return fromFile;
    }
  } catch (error) {
    console.error("Не удалось прочитать data/admin_password.hash:", error.message);
  }

  // На Render экранируйте $ как $$: $$2b$$12$$...
  const fromEnv = String(process.env.ADMIN_PASSWORD_HASH || "").trim();
  if (fromEnv.startsWith("$2")) return fromEnv;
...
if (!SESSION_SECRET || SESSION_SECRET.length < 32) {
  console.error("Ошибка: задайте длинный SESSION_SECRET в Environment / .env");
  process.exit(1);
}

if (!ADMIN_PASSWORD_HASH) {
  console.error("Ошибка: не найден bcrypt-хеш пароля админа.");
  console.error("Нужен файл data/admin_password.hash или корректный ADMIN_PASSWORD_HASH.");
  process.exit(1);
}
